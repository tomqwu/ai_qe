...
**Outcome.** Task-level gains exist in vendor and lab studies; the only independent RCT is negative; telemetry shows stability and defect-rate degradation; consultancies report 10-15% and say it is rarely monetized. No source supports a QA capacity figure or an audited hard-dollar saving. The earlier planning ranges were subsequently withdrawn as client guidance in v1.23.0. No first-year or mature savings forecast is established. Economic-route thresholds are proposed pilot decisions, not industry results; ranking-only questionnaire answers leave activity share unknown. Use the current [savings method](../economics/savings-model/) and explicit scenario inputs.
