...
**Quality and security baselines.** Escaped defects by severity, reopen rate, change-failure rate, rollbacks, production incidents; security-finding volume by scanner, false-positive share, remediation backlog age, SLA attainment.
...
|---|---|---|
| Toolchain inventory (requirements, source control, CI/CD, test management, automation frameworks, observability, SAST/SCA/DAST/secrets/IaC scanners, AI platforms) | Confirms existing-capabilities-first | List with product names and whether integrated in CI/CD |
...
| Security-finding data for the last two quarters | AppSec baseline | Counts by scanner and severity, false-positive closures, median age, SLA attainment |
