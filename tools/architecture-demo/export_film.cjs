...
(async()=>{const browser=await chromium.launch();let encoder;try{
 ...
 await page.evaluate(()=>window.qeArchitecture.seek(11));
 ...
 for(let i=0;i<duration*fps;i++){
  ...
  if(i%(5*fps)===0){fs.writeFileSync(path.join(proof,`frame-${String(i/fps).padStart(2,'0')}.jpg`),frame);console.log(`Rendered ${i/fps}/${duration} seconds`);}
 }
 encoder.stdin.end();const [code]=await done;if(code!==0)throw new Error(encoderError);
 const stamp=t=>`${String(Math.floor(t/3600)).padStart(2,'0')}:${String(Math.floor(t/60)%60).padStart(2,'0')}:${String(t%60).padStart(2,'0')}.000`;let t=0,vtt='WEBVTT\n\n';for(const s of steps){vtt+=`${stamp(t)} --> ${stamp(t+s.duration)}\n${s.title}. ${s.body}\n\n`;t+=s.duration;}fs.writeFileSync(path.join(root,'assets/video/assurance-architecture.vtt'),vtt);
 const inputs=['assets/data/architecture-demo.json','assets/models/assurance-platform.glb','assets/models/assurance-platform.blend','tools/architecture-demo/build_scene.py','tools/architecture-demo/main.js','assets/css/architecture-3d.css','demos/architecture.html'];
