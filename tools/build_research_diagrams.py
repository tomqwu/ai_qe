...
class SVG:
 ...
 def path(self,d,cls='edge',arrow=True):self.parts.append(f'<path d="{d}" class="{cls}"'+(f' marker-end="url(#{self.name}-arrow)"' if arrow else '')+'/>')
...
s.path('M 246 283 H 274');s.path('M 484 283 H 512')
s.path('M 380 113 V 210');s.text(295,153,'task + scope','edge-label')
s.path('M 710 113 V 140 H 138 V 212','edge edge-dashed');s.text(444,155,'approved context','edge-label')
...
s.path('M 724 244 H 790');s.text(738,231,'allow','edge-label','middle')
s.path('M 1028 113 V 155 H 1169 V 325 H 1166','edge edge-dashed')
...
s.path('M 360 516 H 386');s.path('M 620 357 V 417 H 196 V 475','edge edge-dashed');s.text(634,417,'run traces','edge-label')
...
s.path('M 1128 277 V 410 H 1040 V 446');s.path('M 898 358 V 446');s.text(806,431,'checks + evaluation evidence','edge-label')
s.path('M 979 557 V 570 H 196 V 558','edge edge-dashed');s.path('M 724 517 H 757 V 379 H 979 V 360','edge edge-dashed')
