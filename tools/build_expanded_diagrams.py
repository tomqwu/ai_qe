...
s.node(20, 213, 330, 123, 'Shared platform team', ('Context, identity and runtime', 'Reliable service + evidence export'), 'node')
...
s.node(850, 213, 330, 123, 'Security and risk partners', ('Threat cases + control challenge', 'Applicability and escalation advice'), 'node node-sand')
...
s.node(20,70,290,113,'System inventory',('Purpose + critical dependencies','Named accountable owner'), 'node node-teal')
...
s.node(20,55,240,124,'Approved sources',('Contracts + source code','Synthetic fixtures','Access + retention labels'))
s.node(325,55,240,124,'Retrieval service',('User and agent scope','Filter by current access','Select relevant passages'), 'node node-teal')
...
s.node(20,58,270,121,'Fault concern',('Fault: accepts negative value','Select a relevant mutation'), 'node node-sand')
s.node(390,58,310,121,'Candidate test',('Generate for the stated concern','Preserve the intended assertion'), 'node node-teal')
s.path('M 290 119 H 388')
s.node(860,25,320,110,'Original implementation',('Candidate passes','Expected behavior remains intact'))
s.node(860,210,320,110,'Mutated implementation',('Candidate fails for the target fault','Exclude invalid / equivalent faults'))
...
s.node(335,55,260,112,'Diagnostic assistant',('Extract relevant log lines','Propose cause + evidence'), 'node node-teal')
...
s.node(20,65,250,118,'Reproduce the flake',('Capture failure conditions','Retain original test intent'))
s.node(330,65,250,118,'Targeted context',('Dynamic call paths','Timing and shared state'))
s.node(640,65,250,118,'Candidate repair',('Fix the failure mechanism','Preserve valid assertions'), 'node node-teal')
s.node(950,65,230,118,'Validation',('Repeat across conditions','Review behavior change'))
...
s.node(20,60,280,141,'Candidate cases',('Tasks + approved examples','Reviewed production failures','Synthetic edge cases'))
...
s.node(20,60,275,133,'Comparable runs',('Same held-out corpus','Configuration versions','Repeated runs where needed'))
