...
try {
 ...
 const search=await (await page.request.get(`${base}/assets/js/search-data.json`)).json();assert.equal(Object.values(search).filter(x=>/\/briefings\/fintech-.*\/#slide-/.test(x.url)).length,30);
