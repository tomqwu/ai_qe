...
Use `make check` for the full local validation pipeline. Serve `_site` under `/ai_qe/` on port 61600 while browser checks run, or set `QE_TEST_URL`. `make browser GROUP=playback` runs one CI-identical group. Set `PYTHON` to a runtime with `tools/requirements.txt` installed.

## Repository and audience surfaces
...
The homepage links to four full-size audience stories. The industry pair is:
...
`assets/js/briefing-embed.js` switches audiences in the presentation room and the banking case preview.
...
Only an explicit new site edition deploys and publishes a GitHub release. Review, tooling and maintainer changes run validation without attempting to republish an existing tag. Public content changes without a new edition are rejected on main before deployment. `tools/prepare_release.py` assembles the ten current PDFs (four full decks, four guided routes, research and questionnaire), MP4, captions, CSV/JSON source registers and checksums in an empty output directory. `tools/publish_release.py` uploads a draft using the job's repository-scoped token, verifies GitHub's SHA-256 digests, and publishes only after all assets are complete. An existing published edition is never overwritten; increment the site version for a new commit. A matching interrupted draft can resume.
...
## Review gates and outcome-led pilots

`tools/pilot-decision.cjs` applies a sponsor-approved charter, using the primary objective in `_data/pilot_gates.json`. Start from the blank `assets/examples/pilot-charter.json`; the method in `docs/method/phased-pilot.md` defines the evidence contract. Effort thresholds apply to economic cases. Other objectives use their agreed metric, target, exposure and evidence reference. Cash claims additionally need Finance-validated capture. Quality floors, incomplete evidence and cost limits apply to every route.

The existing banking comparison includes three optional observation records. `FintechModel.compare` separates modernization from incremental AI effort, requires matched scope, acceptance criteria and application build, and holds missing controls as unknown. AI comparisons also match the environment, fixtures and provider version. Review, rework, failed attempts, setup, operating hours and evidence references are retained in the downloadable worksheet. Entries are unverified observations; illustrative capacity profiles are not controls.

After building and finalizing `_site`, run `node tools/narration-review.cjs --site _site --candidates /tmp/narration-candidates.json`. The gate hashes each rendered slide, inline guide and demo destination along with its script, recording hash and caption-flow definitions. Shared media has a separate review for every destination. It ignores cosmetic styling and slide-footer editions. A changed destination requires a deliberate `retained` or `refreshed` decision with a specific reason; a removed destination requires `removed`. Put only reviewed decisions in a JSON object keyed by the reported destination ID, then run `node tools/narration-review.cjs --site _site --accept /tmp/narration-decisions.json`. Commit `assets/data/narration-review.json`. Never reset the baseline to silence a stale-review failure. Continue running measured media/caption and browser flow checks; the fingerprint requires human review, not automatic semantic agreement.

CI prepares the publication package and performs read-only `tools/preflight_release.py` checks before deployment. Actual lightweight or annotated tag targets, release commit metadata and published asset checksums must agree. The verified package is passed to the post-deployment publisher, which repeats identity checks before publication. A different commit requires a new site version; an identical interrupted release can resume safely.

Focused regression checks: `node tools/check_review_playback.cjs` exercises Auto-next, real audio completion, lifecycle captions, failure branches and manual takeover. `node tools/review-model-browser-test.cjs` exercises workflow-specific readiness, the three-state worksheet and no-JavaScript form navigation. Both use `QE_TEST_URL` and run in Chromium and WebKit.
