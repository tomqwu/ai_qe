...
- [Executive strategic vision](https://tomqwu.github.io/ai_qe/briefings/evp/): 25 slides on
...
`_data/visual_story.json` holds the proposed FinTech Client integration nodes, authored PAY-142 artifacts, lifecycle stages, modernization grouping and curated audience routes. Dependency requirements remain canonical in `_data/adoption.json` and are evaluated by `readiness-model.js`; the map adds no competing maturity score. Diagnosis-only review and execution/retest have distinct prerequisites.
